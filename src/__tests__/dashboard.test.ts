...
const filterCandidatesByMinScore = (candidates: any[], minScore: number) => {
  return candidates.filter((c) => c.score >= minScore);
};

const sortCandidatesByScore = (candidates: any[]) => {
  return [...candidates].sort((a, b) => b.score - a.score);
};

const getCandidateCompatibilityLabel = (score: number) => {
  if (score >= 80) return 'Excelente';
  if (score >= 60) return 'Muito Bom';
  if (score >= 40) return 'Bom';
  if (score >= 20) return 'Aceitável';
  return 'Baixo';
};

const shouldShowCandidateFlag = (flagStatus: string) => {
...
describe('Dashboard & Matching', () => {
  ...
  it('should calculate match feedback impact', () => {
    const applyFeedback = (baseScore: number, feedback: 'positive' | 'negative' | 'skip') => {
      if (feedback === 'positive') return baseScore + 20;
      if (feedback === 'negative') return baseScore - 60;
      if (feedback === 'skip') return baseScore - 12;
      return baseScore;
    };
