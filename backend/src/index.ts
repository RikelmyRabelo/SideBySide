...
// Importação do motor de pareamento e WebRTC
...
pubClient.on('error', (err) => logger.error('Erro no Redis pubClient:', err));
subClient.on('error', (err) => logger.error('Erro no Redis subClient:', err));
...
// 2. Middleware de autenticação JWT unificado
io.use((socket: Socket, next: (err?: Error) => void) => {
  try {
    ...
    (socket as any).user = decoded; // Garante o usuário no socket
  ...
  try {
    ...
    await prisma.notification.create({
      data: {
        ...
        message: 'Estamos muito felizes em ter você aqui. Dê o primeiro passo para destravar seu inglês!',
        read: false,
      },
    });
...
app.post('/api/room/report', authenticateToken, reportLimiter, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    logger.warn(`AUDITORIA DE DENÚNCIA: Usuário ${userId} denunciou ${reportedUserId} por "${reason}".`);
...
app.post('/api/friends/request', authenticateToken, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!resolvedTargetId) {
      return res.status(400).json({ error: 'targetUserId ou tag é obrigatório.' });
    }
  ...
  try {
    ...
    return res.status(200).json(
      requests.map((request) => ({
        ...
        tag: request.user.tag || `${request.user.name.replace(/\s+/g, '')}#${request.user.id.slice(0, 4)}`,
...
app.get('/api/friends', authenticateToken, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = req.user!.id;
    ...
    return res.status(200).json(
      friends.map((friend) => ({
        id: friend.id,
        name: friend.name,
        tag: friend.tag || `${friend.name.replace(/\s+/g, '')}#${friend.id.slice(0, 4)}`,
